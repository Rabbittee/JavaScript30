...
var anyPass = function (preds) {
  return function (arg) {
    return preds.some(function (pred) {
      return pred(arg);
    });
  };
};
var lerp = function (x1, x2, t) {
  return x1 * (1 - t) + x2 * t;
};
var percentage = function (x1, x2) {
  return (x1 / x2) * 100;
};
state({
  playing: false,
  ...
  .observe(function (dispatch) {
    ...
    select('.player .progress')
      ...
      .addEventListener('click', function (e) {
        var isPointerOrMouse = anyPass([is(PointerEvent), is(MouseEvent)])(e);
        if (isPointerOrMouse) {
          var _a = e,
  ...
  .reduce(function (state, action) {
    if (action.type === 'toggle') return __assign(__assign({}, state), { playing: !state.playing });
    if (action.type === 'volume') return __assign(__assign({}, state), { volume: action.value });
