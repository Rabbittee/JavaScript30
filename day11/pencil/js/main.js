...
$('#playArea').addEventListener('click', (e) => {
  ...
  currentTitle === 'replay' ? replay(false) : playingStatus(isPaused);
});
...
const setVolIcon = (vol) => {
  ...
  if (vol > 0) {
    vol > 0.5 ? (volBtnImg.src = ICON_HIGH_VOL) : (volBtnImg.src = ICON_LOW_VOL);
  }
};
...
$('input[name="volume"]').addEventListener('input', () => {
  const vol = $('input[name="volume"]').value;
  updateVolState(vol);
});

$('[data-title="volume"]').addEventListener('click', () => {
...
$('.progress__area').addEventListener('click', (e) => {
