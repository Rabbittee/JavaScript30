...
var inRange = function (bound1, bound2) {
  return function (value) {
    var _a = bound1 > bound2 ? [bound2, bound1] : [bound1, bound2],
      min = _a[0],
      max = _a[1];
...
state({
  ...
  .then(function (state, action, dispatch) {
    ...
    var index = state.inputs.findIndex(function (item) {
    ...
    state.inputs
      .filter(function (_, current) {
        return inRange(state.lastIndex, index)(current);
      })
