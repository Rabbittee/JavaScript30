...
function state(value) {
    ...
    function observe(source) {
        queueMicrotask(function () { return source(value, dispatch); });
        return combiner;
    }
...
state({
    ...
    select('.speed').addEventListener('mousemove', function (_a) {
        var target = _a.target, offsetY = _a.offsetY;
    ...
    if (action.type === 'move') {
        return __assign(__assign({}, state), { percent: Math.round(action.payload * 100), rate: lerp(state.range.min, state.range.max, action.payload) });
    }
    return state;
})
    .then(function (_a) {
