...
var setProperty = function (element) {
  return function (property, value) {
    return element.style.setProperty('--'.concat(property), value);
  };
};
var update = function (target) {
  return setProperty(select('body'))(
    target.name,
    ''.concat(target.value).concat(target.dataset.sizing || '')
  );
};
