...
const selectAll = <T extends Element>(query: string) => document.querySelectorAll<T>(query);

const setProperty = (element: HTMLElement) => (property: string, value: string) =>
  element.style.setProperty(`--${property}`, value);

const update = (target: HTMLInputElement) =>
  setProperty(select('body'))(target.name, `${target.value}${target.dataset.sizing || ''}`);
