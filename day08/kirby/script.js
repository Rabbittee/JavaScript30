...
state({
  ...
  .reduce(function (state, event) {
    if (event.type === 'press') return __assign(__assign({}, state), { holding: true });
    if (event.type === 'execute') return __assign(__assign({}, state), { cursor: event.cursor });
    if (event.type === 'release') return __assign(__assign({}, state), { holding: false });
  })
  .then(Drawer());
