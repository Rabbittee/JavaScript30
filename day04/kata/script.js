...
const totalLiveYears = inventors.reduce((prev, curr) => {
  return prev + (curr.passed - curr.year);
}, 0);

console.log(`Total live ${totalLiveYears} years`);

// 5. Sort the inventors by years lived
const sortedByLivedYears = inventors
  .map((inventor) => {
    return {
      ...inventor,
      livedYears: inventor.passed - inventor.year,
    };
  })
  .sort((a, b) => b.livedYears - a.livedYears);
...
const sortedByAlphabet = people.sort((a, b) => {
...
const sumCount = data.reduce((prev, curr) => {
  if (!prev[curr]) {
    prev[curr] = 0;
  }
  prev[curr]++;
