...
var observer = new IntersectionObserver(function (entries) {
    return entries.forEach(function (_a) {
        ...
        return target.dispatchEvent(new CustomEvent("stick-change", { detail: { isIntersecting: isIntersecting } }));
    });
}, { threshold: 1 });
selectAll("nav.sticky")
    .map(tap(function (el) { return observer.observe(el); }))
    .map(tap(function (el) {
    return el.addEventListener("stick-change", function (_a) {
        var target = _a.target, detail = _a.detail;
