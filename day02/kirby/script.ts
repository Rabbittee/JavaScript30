...
every(1000, {
  hour: (time: Date) => setRotate(select('.hour-hand'), lerp(0, 360, time.getHours() / 24)),

  min: (time: Date) => setRotate(select('.min-hand'), lerp(0, 360, time.getMinutes() / 60)),

  second: (time: Date) => setRotate(select('.second-hand'), lerp(0, 360, time.getSeconds() / 60)),
});
